...
# May 2020, fixing a bug with empty factor levels and reference levels
...
#  "PostAIC intervals" either for each parameter appearing in the selected model or for the linear combinations.
  ...
  Xfull<-droplevels(Xfull)
  
  nfactors<-which(sapply(1:ncol(Xfull), function(i) is.factor(Xfull[,i]))==T)
  for(i in 1:length(nfactors)){
    contrasts( Xfull[,nfactors[i]])<- contr.treatment(levels(Xfull[,nfactors[i]]),base=1)
  }
  
  data.full=cbind(y,Xfull)
  fit.full = glm(y~.-1,offset=offset,family=family, data=data.full)
  mmfull<-model.matrix(fit.full)
  ncfull<-attr(mmfull,"assign")
  
  nac<-which(is.na(coef(fit.full)))
  if(length(nac)>0){
    mmfull<-mmfull[,-nac]
    lcoef<-length(fit.full$coefficients[-nac])
    ncfull<-ncfull[-nac]
  }else{
    lcoef<-length(fit.full$coefficients)
  }
  namesfull<-colnames(mmfull)
  
  
  Pi.new2<-sapply(ncfull, function(x) Pi[,x])
  
  
  aic<-vector()
  for (k in 1:Mn){
    Xm<-mmfull[,which(Pi.new2[k,]!=0)]
    model = glm(y~Xm-1,offset=offset,family=family)
    aic[k] = AIC(model)
  }
  
  pselected= which.min(aic)
  
  X.aic=mmfull[,which(Pi.new2[pselected,]!=0)]
  fit.aic= glm(y~X.aic-1, offset=offset, family=family)
  summary.fit=summary(fit.aic)$coefficients
  rownames(summary.fit)<-namesfull[which(Pi.new2[pselected,]!=0)] 
  ...
  if (length(y)<30){
    Jfull=solve((vcov(fit.full,complete = F)*(length(y)-lcoef)/length(y))*length(y))  
...
if(linearcomb==FALSE){
  ...
  names(coefs)<-namesfull[which(Pi.new2[pselected,]!=0)] 
